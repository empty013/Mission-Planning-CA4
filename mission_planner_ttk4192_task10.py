...
def main_hybrid_a(heu,start_pos, end_pos,reverse, extra, grid_on):
    ...
    def animate(i):
        ...
        if i < len(branches):
            ...
            # ax.plot(xl[min(j, len(path)-1):], yl[min(j, len(path)-1):])
            ...
            # ax.plot(xl[:min(j+1, len(path))], yl[:min(j+1, len(path))])
            _path1.set_zorder(3)
...
class turtlebot_move():
    ...
    def __init__(self):
        ...
        for point in WAYPOINTS_MOVE[1::2]:
            # Ugly but necessary
            # self.move_to_point(point[0] * 0.1, point[1] * 0.1)
            print(f"control_points: {point[0]},   {point[1]}")
            print(len(WAYPOINTS_MOVE))
...
class TakePhoto:
    def __init__(self):
        # rospy.init_node('takepicture', anonymous=False)
...
def taking_photo_exe():
    ...
    file_source = os.path.expanduser('~/catkin_ws/')
    # file_destination = '~/catkin_ws/src/assigment4_ttk4192/scripts'
    file_destination = os.path.expanduser('~/catkin_ws/src/assigment4_ttk4192/scripts')
    g='photo'+dt_string+'.jpg'
    # open(os.path.expanduser('~/catkin_ws/src/AI-planning/tmp_sas_plan.1'))
